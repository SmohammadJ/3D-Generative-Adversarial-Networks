...
def main():
    ...
    mnist_3d = Minst3D(transform=transforms.Compose([
                       transforms.Resize( ( 32, 32 ) ),
                       transforms.ToTensor(), 
                   ]))

    train_loader = torch.utils.data.DataLoader(mnist_3d, batch_size=opt.batch_size, shuffle=True)
    ...
    for epoch in range( opt.n_epochs ):
        ...
        for i, ( voxels, _ ) in enumerate( train_loader ):
            ...
            if batches_done % opt.sample_interval == 0:
                ...
                for i in range (2):  
                    gen_voxel_show = gen_voxels[i,0].cpu().detach().numpy()  
                    fig = plt.figure()
                    ax = fig.gca(projection='3d')
                    ax.voxels(gen_voxel_show,  edgecolors='k')
                    plt.savefig('voxels{}_{}.png'.format(batches_done,i))
